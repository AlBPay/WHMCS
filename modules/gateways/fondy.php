...
{
    return array(
        'DisplayName' => 'Alliance bank - Online payments',
...
function fondy_config()
{
    return array(
        ...
        'FriendlyName' => array(
            'Type' => 'System',
            'Value' => 'Alliance bank - Online payments',
        ),
...
function fondy_link($params)
{
    ...
    $url = 'https://payment.albpay.io/api/checkout/redirect/';
...
function doFondyRefund($refundAmount, $reason, $transactionIdToRefund, $secretKey, $accountId, $currencyCode)
{
    ...
    try {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://payment.albpay.io/api/reverse/order_id');
